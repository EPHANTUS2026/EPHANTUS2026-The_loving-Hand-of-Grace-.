...
import ReferenceFinder from '@/components/knowledge/ReferenceFinder';
...
export default async function Page(){
 ...
 <section className="section"><div className="container-page space-y-12"><DailyRecovery/><ReferenceFinder/>
  ...
  <div id="aa-literature"><div className="mb-5"><div className="text-xs font-black uppercase tracking-[.16em] text-grace-700">Recovery Literature · Alcoholics Anonymous</div><h2 className="mt-2 text-2xl font-black text-slate-950">AA Recovery Books</h2><p className="mt-2 text-sm text-slate-600">Direct access to official Alcoholics Anonymous World Services book resources. Loving Hand of Grace links to the official publisher and does not reproduce the books.</p></div><div className="grid gap-5 md:grid-cols-2">{[RECOVERY_RESOURCES.aaBigBook,RECOVERY_RESOURCES.aaTwelveAndTwelve].map(resource=><article key={resource.id} className="card flex flex-col"><h3 className="text-xl font-black text-slate-950">{resource.title}</h3><p className="mt-1 text-sm font-semibold text-grace-700">{resource.subtitle}</p><p className="mt-3 flex-1 text-sm leading-6 text-slate-600">{resource.description}</p><a href={resource.officialUrl} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-grace-800 px-5 py-3 font-bold text-white focus:outline-none focus:ring-2 focus:ring-grace-500 focus:ring-offset-2" aria-label={'Open '+resource.title+' on the official Alcoholics Anonymous website — opens in a new tab'}>Read Official Book <ArrowTopRightOnSquareIcon aria-hidden="true" className="h-4 w-4"/></a><p className="mt-3 text-xs text-slate-500">Official Alcoholics Anonymous World Services resource · external website</p></article>)}</div></div>
