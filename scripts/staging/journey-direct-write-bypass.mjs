...
const assertAuditUnchanged=async(id,before,label)=>{const row=(await restSelect('audit_log',`id=eq.${id}&select=id,reason`))[0];if(!row)throw new Error(`${label} removed audit evidence`);if(row.reason!==before)throw new Error(`${label} mutated audit evidence`);};
...
// RLS can intentionally hide audit rows from this role, causing PostgREST to return 200 with zero affected rows.
// Certification therefore proves the invariant by re-reading through trusted test authority rather than equating HTTP status with mutation.
await userFetch(`/rest/v1/audit_log?id=eq.${audit.id}`,{method:'PATCH',body:{reason:'forged'}});await assertAuditUnchanged(audit.id,audit.reason,'audit evidence rewrite');
await userFetch(`/rest/v1/audit_log?id=eq.${audit.id}`,{method:'DELETE'});await assertAuditUnchanged(audit.id,audit.reason,'audit evidence deletion');
