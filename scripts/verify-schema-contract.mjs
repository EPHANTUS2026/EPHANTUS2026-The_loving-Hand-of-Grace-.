const required={
  ...
  rpc:['get_available_booking_slots','hold_booking_slot','publish_meditation_if_approved']
};
...
// RPC existence is verified by a deliberately invalid/null invocation: 404 means absent; any other response proves the route exists.
for(const fn of required.rpc){const r=await fetch(`${url}/rest/v1/rpc/${fn}`,{method:'POST',headers:{apikey:key,Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:'{}'});if(r.status===404) missing.push(`rpc:${fn}`);}
