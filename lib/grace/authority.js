...
export function requireStaffRelationship({identity,assignment,purpose,scope,clientId,now=new Date()}){
  assertProtectedModeAuthority(identity);
  ...
  if(clientId&&assignment.client_id!==clientId) throw new Error('Staff client relationship denied.');
  if(assignment.revoked_at || (assignment.starts_at&&new Date(assignment.starts_at)>now) || (assignment.ends_at&&new Date(assignment.ends_at)<=now)) throw new Error('Staff assignment is not current.');
