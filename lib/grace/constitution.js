...
/**
 * GRACE-1 permanent constitutional contract.
 * These values are policy inputs for server-side orchestration and tests; they
 * are not a substitute for database/RLS authorization.
 */
export const GRACE_CONSTITUTION = Object.freeze([
  'COMPASSION',
  'DIGNITY',
  'SAFETY',
  'TRUTHFULNESS',
  ...
  'LEAST_PRIVILEGE',
  'DATA_MINIMISATION',
  'HUMAN_CLINICAL_AUTHORITY',
]);

export const GRACE_OPERATING_MODES = Object.freeze({
  PUBLIC: { authenticated:false, protectedContext:false },
  CLIENT: { authenticated:true, protectedContext:true, authority:'SELF_ONLY' },
  FAMILY: { authenticated:true, protectedContext:true, authority:'CONSENT_SCOPED_RELATIONSHIP' },
  STAFF: { authenticated:true, protectedContext:true, authority:'ROLE_ASSIGNMENT_SCOPE_PURPOSE' },
  AFTERCARE: { authenticated:true, protectedContext:true, authority:'AUTHORIZED_RECOVERY_CONTEXT' },
  SAFETY: { authenticated:false, protectedContext:false, overridesConversation:true },
});
...
export const CLINICAL_AUTHORITY = Object.freeze({
  principle:'Grace may support clinical work. Grace does not become the clinical authority.',
  consequentialOutcome:'HUMAN_REVIEW_REQUIRED',
  prohibitedIndependentActions:[
    'DIAGNOSE',
    'PRESCRIBE_OR_MODIFY_MEDICATION',
    'DECLARE_MEDICAL_SAFETY',
    'FINAL_CLINICAL_RISK_CLASSIFICATION',
    'AUTHORIZE_CLINICAL_ADMISSION',
    'AUTHORIZE_DISCHARGE',
    'INVOLUNTARY_TREATMENT_DECISION',
    'OVERRIDE_QUALIFIED_PROFESSIONAL',
  ],
});

export function resolveGraceOperatingMode({safetyLevel='STANDARD',session=null,requestedMode=null}={}){
  if(safetyLevel==='EMERGENCY') return 'SAFETY';
  const role=String(session?.profile?.role||'').toLowerCase();
  if(!session?.profile?.is_active) return 'PUBLIC';
  if(role==='client') return requestedMode==='AFTERCARE'?'AFTERCARE':'CLIENT';
  if(role==='family') return 'FAMILY';
  if(['counsellor','clinician','clinical_director','doctor','nurse','psychologist','social_worker','case_manager','aftercare_coordinator','family_liaison','staff','director','admissions','administrator','manager','super_admin'].includes(role)) return 'STAFF';
  return 'PUBLIC';
}

export function clinicalAuthorityDecision(intent){
  const clinical=new Set(['DIAGNOSIS_REQUEST','MEDICATION_OR_MEDICAL_REQUEST','CLINICAL_DECISION_REQUEST']);
  return clinical.has(intent)
    ? {clinical:true,decision:'HUMAN_REVIEW_REQUIRED',mayExecute:false}
    : {clinical:false,decision:'NOT_REQUIRED',mayExecute:true};
}
