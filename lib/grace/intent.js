const INTENTS={
  ...
  PROGRAMME_INFORMATION:[/\b(programme|program|residential|outpatient|treatment options?|services do you offer|what do you offer)\b/i,/\b(programu|huduma mnazotoa|matibabu gani)\b/i],
  ...
  CLINICAL_DECISION_REQUEST:[/\b(which|what)\s+(programme|program|treatment)\s+.*(need|best|right|for me|for him|for her)\b/i,/\b(should .* be admitted|do i need residential|does .* need treatment)\b/i,/\b(ni programu gani.*inafaa|matibabu gani.*nahitaji)\b/i],
  DIAGNOSIS_REQUEST:[/\b(diagnos|am i an alcoholic|am i addicted|do i have (addiction|depression|anxiety|bipolar))\b/i,/\b(je nina uraibu|nitambue ugonjwa)\b/i],
...
export function classifyIntent(message='',options={}){
  ...
  let primary='UNKNOWN';
  if(meaningful.length) primary=meaningful[0][0];
  else if(hasGreeting) primary='GREETING';
  else if(scoreIntent('SMALL_TALK',text)>0) primary='SMALL_TALK';
  if(emotionalState!=='NONE' && !['CRISIS_OR_IMMEDIATE_DANGER','SAFETY_CONCERN'].includes(primary)) primary='EMOTIONAL_SUPPORT';
  ...
  const topScore=scores.find(([intent])=>intent===primary)?.[1]||1;
  const confidence=primary==='UNKNOWN'?0.35:Math.min(0.99,0.72+(topScore-1)*0.1+(primary==='GREETING'?0.2:0));
