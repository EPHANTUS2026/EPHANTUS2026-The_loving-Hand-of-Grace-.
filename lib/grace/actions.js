...
export async function createGraceFlowAction(input={}){
  ...
  // GraceFlow remains authoritative. Replays resolve to the already-confirmed workflow.
  const existing=(await dbAdminSelect('workflow_instances',`metadata->>idempotency_key=eq.${encodeURIComponent(idempotencyKey)}&metadata->>source=eq.grace&select=id,workflow_key,current_state,status,entity_id&limit=1`))?.[0];
  if(existing){
    return {confirmed:true,replayed:true,requestId:existing.entity_id,workflowInstanceId:existing.id,workflow:existing.workflow_key,state:existing.current_state,status:existing.status,message:'This request was already received. GraceFlow confirmed the existing authoritative request.'};
  }
  ...
  const instanceRows=await dbInsert('workflow_instances',{
    ...
    metadata:{source:'grace',context,action,consent:true,contact,idempotency_key:idempotencyKey,command_id:clean(input.commandId,80)||null,actor:input.actor||null}
  });
  ...
  await dbInsert('workflow_events',{
    ...
    metadata:{request_id:requestId,action,context,idempotency_key:idempotencyKey,command_id:clean(input.commandId,80)||null}
  });
  ...
  const taskRows=await dbInsert('workflow_tasks',{
    ...
    payload:{request_id:requestId,source:'grace',context,contact,note:clean(input.note,500)||null,consent:true,idempotency_key:idempotencyKey,actor:input.actor||null}
  });
