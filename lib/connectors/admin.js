...
export async function testConnector(channel,{enable=false,actor=null}={}){
  ...
  await dbUpdate('integration_connectors',`connector_key=eq.${encodeURIComponent(channel)}`,{enabled:enable?Boolean(health.ok):undefined,last_health_status:status.toLowerCase(),last_health_at:new Date().toISOString()});
